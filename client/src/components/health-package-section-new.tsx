    ...
    <section id="health-packages" className="py-32">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <h3 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold text-gray-900 mb-4">건강구독 테마별 패키지</h3>
          <p className="text-sm sm:text-base md:text-lg text-gray-600 max-w-2xl mx-auto mb-2">
              ...
              <Card key={theme} className={`bg-gradient-to-br ${colors.card} border-2 transition-all duration-300 hover:shadow-xl`}>
                <CardContent className="p-10">
                  <div className="text-center">
                    ...
                    {/* 테마 제목 */}
                    <h4 className={`text-lg sm:text-xl md:text-2xl lg:text-3xl font-extrabold ${colors.text} mb-8 tracking-tight`}>{theme}</h4>
                    ...
                    {selectedPackage?.theme === theme && selectedPackageData && (
                      <div className={`mt-4 p-6 rounded-xl border-2 animate-in slide-in-from-top duration-300 ${
                        selectedPackage.type === 'premium' 
                          ...
                          : `bg-${theme === '면역건강구독' ? 'green' : theme === '해독다이어트구독' ? 'orange' : 'pink'}-50 border-${theme === '면역건강구독' ? 'green' : theme === '해독다이어트구독' ? 'orange' : 'pink'}-200`
                      }`}>
